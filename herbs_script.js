(function() {
  ...
  Layout.initBlogBlock = function() {
    ...
    tb.ajax.loadDocument("/cache/magazin/").then(function($page) {
      ...
      $("#newsWrapper .news-item").each(function() {
        $(this).find("a.title").clone().insertAfter($(this).find(".text")).html("Celý èlánek").addClass("arrow");
  ...
  Cart.cartContentLoaded = function(snippets) {
    ...
    $("body.id--9 .sidebar-in-cart .btn").text("Pokraèovat v objednávce");

    $('<span class="show-tooltip acronym" title="" data-testid="cartItemDiscount" data-original-title="Pøi objednání do 10:00 odesíláme ještì ten samý den"><span class="tooltip-icon"></span></span>').prependTo(".delivery-time");
    ...
    $("<tr class='top'><td colspan='2'>Položka košíku</td><td>Dostupnost</td><td>Množství</td><td>Cena za kus</td><td>Souèet</td></tr>").prependTo("body.in-kosik .cart-table:not(.with-disc) tbody");
    $("<tr class='top'><td colspan='2'>Položka košíku</td><td>Dostupnost</td><td>Množství</td><td>Cena za kus</td><td>Sleva</td><td>Souèet</td></tr>").prependTo("body.in-kosik .cart-table.with-disc tbody");
    ...
    $("body.in-krok-1 .next-step .next-step-forward").text("Pokraèovat v objednávce");
    ...
    $(".login-form-group label").html("Máte u nás úèet? Pøihlaste se a nemustíte vyplòovat své údaje. <span id='login-here'></span>").removeClass("btn btn-secondary");
    $(".login-form-group a").removeClass("btn btn-secondary").text("Pøihlásit se").appendTo("#login-here");
    ...
    $("<div class='consents-last'>Beru na vìdomí, že zaplacenou objednávku <b>již není možné dodateènì upravovat èi mìnit.</b></div>").prependTo("body.in-krok-2 .next-step");
    ...
    $("#orderFormButton").text("POKRAÈOVAT").appendTo(".order-summary-inner");
  ...
  Catalog.initAdvancedOrder = function(snippets) {
    ...
    if($modal.find(".advanced-order-suggestion").length)
      $modal.addClass("--has");
    $("<div onclick='shoptet.modal.close();' class='extra btn btn-secondary btn-back'>Zpìt do obchodu</div>").prependTo(".advanced-order .extras-wrap");
    $(".advanced-order .extras-wrap").append(tb.$h(".extra.step", $(".advancedOrder__buttons").detach().find(".btn-conversion")));
    $(".advanced-order .extras-wrap .extra.step .btn").html("Pøejít do košíku").addClass("btn-next");
  ...
  const Detail = (function($h, $t, Banners) {
      ...
      return $h(".pagination-wrapper", [
        $h(".pagination", [
          ...
          page > 0 ? PageLink(page - 1, [
            $h("span.sr-only", "Pøedchozí strana"),
    ...
    function initDetail($detail) {
      ...
      $wrappedInfo.find(".delivery-time-label").text("Objednávku doruèíme nejpozdìji do");
      ...
      $("<div class='extra-discount'><b>Extra sleva 5 %</b> pro zaregistrované uživatele. <a href='/login/'>Pøihlásit se.</a> Nemáte úèet? <a href='/registrace/'>Registrujte se</a></div>").appendTo($wrappedInfo);
      $("<div class='badge-row'><b>Garance vrácení penìz (30 dní).</b> Pokud vám produkt nebude z nìjakého dùvodu vyhovovat, mùžete nám ho vrátit. <a href='/garance-vraceni-penez/'>Více informací</a></div>").appendTo($wrappedInfo);
      ...
      $detailParameters.find("th:contains('Poèet šálkù')").parent().addClass("add-row --coffe").appendTo($appendixWrapper);
      ...
      $detailParameters.find("th:contains('Dodateèný název')").parent().addClass("product-appendix").prependTo($appendixWrapper);
      ...
      const $moreInfo = $detailParameters.find("th:contains('Doplòující informace')").parent().hide();
      const moreInfoHtml = $moreInfo.find("td").text();
      if(moreInfoHtml)
        $imageWrapper.append(tb.$h(".more-info-link", [moreInfoHtml]));

      $detailParameters.find("th:contains('Doplòující informace')").parent().addClass("add-row --more-info").prependTo($appendixWrapper);
      ...
      $("<div class='gift__heading'><span>Dárek ZDARMA</span> pøi objednávce <b>alespoò 2 balení</b></div>").prependTo($detail.find(".p-gift-name.--ash"));

      /*$('<div class="p-gifts-wrapper">\
        <div class="p-gifts-heading"></div>\
        <div class="p-gift">\
        <a href="/horcik/" class="p-gift-image">\
        <img src="/user/documents/upload/horcik-zdarma.jpg" alt="Horèík">\
        </a>\
        <div class="p-gift-name"><div class="gift__heading"><span>Dárek ZDARMA</span></div>\
        <a href="/horcik/">+ Hoøèík s vysokou vstøebatelností <span class="nowrap">v hodnotì 480 Kè</span>\
                    </a>\
                </div>\
            </div>\
      </div>').prependTo($detail.find(".spec-product .p-info-wrapper"));*/
      //$("<div class='gift__heading'><span>Dárek ZDARMA</span> pøi objednání <strong>alespoò 2 balení</strong></div>").prependTo($detail.find(".p-gift-name"));
        ...
        $h(".info-block", [
          ...
          $h(".stars-block", [
            ...
            $h(".info-block__description", [
              $h("span", "Zákazníkù nás doporuèuje"),
              " ",
              $h("a", { href: "/hodnoceni-obchodu/", target: "_blank" }, "Pøeèíst recenze"),
            ]),
          ]),
          $h("a.heureka-block-new__btn", { href: "/hodnoceni-obchodu/" }, "Zobrazit všechna hodnocení"),
        ]),
        $h("h2", "Další hodnocení zákazníkù"),
        $h(".heureka-inner"),
    ...
    const config = {
      ...
      trans: {
        cs: {
          ...
          flagsParameterName: "Pøíznaky varianty",
        }
      }
    };
  ...
  const CartUtils = (function($h, $t, AjaxSubmitFormHooks) {
    const config = {
      ...
      trans: {
        cs: {
          cartUrl: "/kosik/",
          amountLocked: "Množství nelze mìnit, protože je navázané na ostatní položky."
        },
        en: {
          cartUrl: "/cart/",
  ...
  const DetailUpsell = (function($h, $t, ProductInfo, CartUtils, AdvancedOrderHooks) {
    const config = {
      ...
      trans: {
        cs: {
          inStockRegex: /skladem/i,
          simpleParameterNames: ["Doplòkový produkt", "Doplòkové produkty"],
          urlParameterName: /Doplòkový produkt ([0-9]+)/,
          nameParameterName: /Název doplòkového produktu ([0-9]+)/,
          descriptionParameterName: /Popis doplòkového produktu ([0-9]+)/,
        },
        sk: {
          inStockRegex: /na sklade|skladom|skladem/i,
          simpleParameterNames: ["Doplòkový produkt", "Doplòkové produkty", "Doplnkový produkt", "Doplnkové produkty"],
          urlParameterName: /Dopl[òn]kový produkt ([0-9]+)/,
          nameParameterName: /Náz[eo]v dopl[òn]kového produktu ([0-9]+)/,
          descriptionParameterName: /Popis dopl[òn]kového produktu ([0-9]+)/,
        }
      },
      hideOutOfStock: true,
    };
    ...
    const init = tb.dom.createModuleInitializer({
      ...
      earlyRender() {
        tb.dom.$eachOneTime(".p-detail", async $detail => {
          ...
          $h(".he-detail-upsell", [
            $h(".he-detail-upsell__title", "Pøidejte ještì výhodnì k objednávce:"),
      ...
      rules: [
        ...
        },
        {
          ...
          transformContent(html) {
            ...
            tb.$h(".close-btn-wrap", [
              tb.$h("button.btn.btn-secondary.close-btn", { type: "button", onclick: "shoptet.modal.close()" }, "Zavøít")
  ...
  const HerbsBits = (function($h, $t, AdvancedOrderHooks, Banners, NormalizeMobileHtml, CategorySortSelect, AdvancedOrderProductAppendix) {
    const config = {
      ...
      trans: {
        cs: {
          ...
          hpCatsSmallHeading: "Pøírodní doplòky stravy pøímo pro vás",
    ...
    function initAoProductBlock($p) {
      ...
      return tb.ajax.loadDocument(url).then($doc => {
        const $detail = $doc.find(".p-detail");
        const $detailParameters = $detail.find(".extended-description .detail-parameters");
        const appendix = $detailParameters.find("th:contains('Dodateèný název')").parent().find("td").text();
        if(!appendix)
          return;
    ...
    const BlogBlock = ({ url, title, limit = 3 }) => {
      ...
      tb.ajax.loadDocument(url).then($doc => {
        ...
        $inner.html($h("#newsWrapper.news-wrapper", tb.dom.$filter($items.slice(0, limit), $item => {
          ...
          $title.clone().html("Celý èlánek").addClass("arrow")
      ...
      return $h(".blog-block.--index", [
        ...
        $h(".blog-block__buttons", [
          $h("a.blog-block__btn", { href: url }, "Zobrazit všechny èlánky"),
        ]),
      ]);
    };

    const CategoryUsps = () => $h(".he-cat-usps", [
      $h(".he-cat-usp.--delivery-truck", $h("span", "Doprava do 2 pracovních dnù")),
      $h(".he-cat-usp.--flag-cz", $h("span", "Vyrobeno a vyvinuto v ÈR")),
      $h(".he-cat-usp.--badge", $h("span", "Garance vrácení penìz")),
    ]);
    ...
    const init = tb.dom.createModuleInitializer({
      ...
      earlyInit() {
        ...
        if($("body.type-index").length) {
          ...
          // Nejnovìjší èlánky z magazínu
          $content.append(BlogBlock({ url: t.hpBlogUrl, title: "Nejnovìjší èlánky z magazínu", }));
          // Recenze
          // Instagram
      ...
      earlyRender() {
        ...
        $(".category-top h4, #productsTop").remove(); // nejprodávanìjší
  ...
  const LabWidget = (function(GoogleMaps, CartItemData, CartUtils, CheckoutUtils) {
    ...
    function getPlaceName(place) {
      return place.properties.name
        .replace(/^Odbìrové pracovištì /, "")
        .replace(/^POUZE E-shop - odbìry krve - /, "");
    }
    ...
    function parseOpeningHours(rawHours) {
      const weekdays = ["Pondìlí", "Úterý", "Støeda", "Ètvrtek", "Pátek", "Sobota", "Nedìle"];
    ...
    const PlaceModalForm = ({ onSuccess, }) => {
      ...
      async function handleSubmit(e) {
        ...
        try {
          ...
          console.error(err);
          errorMessage.val = "Nepodaøilo se urèit polohu zadané adresy.";
        }
      }
      async function queryGeolocation(e) {
        ...
        try {
          ...
          console.error(err);
          errorMessage.val = "Nepodaøilo se urèit Vaší polohu.";
        } finally {
      ...
      return h("form.he-lw-modal__search-form", { onsubmit: handleSubmit, }, [
        h(".he-lw-modal__search-form-group", [
          ...
          h("input.form-control.he-lw-form-control", { type: "text", name: "address", required: true, placeholder: "Napø. Praha", }),
          h("button", {
            type: "submit",
            title: "Vyhledat",
          }),
    ...
    const PlaceModalSidebar = ({ places, newPlace, onSelect, onClose, onSearch, }) => h(".he-lw-modal__sidebar", [
      h(".he-lw-modal__header", [
        h(".he-lw-modal__title", "Odbìrová místa"),
    ...
    const PlaceModalSelected = ({ place, onConfirm, onCancel, }) => {
      ...
      const renderOpeningHours = parsedOpeningHours => h(".he-lw-modal__selected-hours",
        parsedOpeningHours.days.map(({ shortName, periods, closed, }, i) => [
          i ? h("br") : null,
          `${shortName}: ${closed ? "zavøeno" : periods.join(", ")}`,
        ]));
    ...
    const DetailsModal = ({ details, onClose, onSubmit, }) => {
      ...
      const formDom = h("form.he-lw-modal__content", { onsubmit: handleSubmit }, [
        h(".he-lw-modal__title", "Koho budeme vyšetøovat?"),
        h(".he-lw-modal__details-form", [
          ...
          FormGroup({ label: "Pøíjmení", required: true }, ({ id, required }) =>
          ...
          FormGroup({ label: "Rodné èíslo", required: true }, ({ id, required }) =>
      ...
      return h(".he-lw__parameter.--place", {
        ...
        h(".he-lw__parameter-left", [
          () => van.val(place)
            ...
            : h(".he-lw__places-prompt", "Seznam odbìrových míst"),
          () => van.val(place)
        ...
        h("a.he-lw__parameter-link", { href: "#", }, () => van.val(place) ? "Zmìnit místo" : "Vybrat odbìrové místo"),
      ]);
    };
    ...
    const DetailsParameter = ({ details, disabledReason, onClick, }) => {
      return h(".he-lw__parameter.--details", {
        ...
        h(".he-lw__parameter-left", [
          h("a.he-lw__parameter-name", { href: "#" }, "Koho budeme vyšetøovat?"),
        ...
        () => van.val(details) ? h("a.he-lw__parameter-link", { href: "#" }, "Zmìnit údaje") : "",
      ]);
    };
    ...
    const LabWidget = ({ ...props } = {}) => {
      ...
      const dom = h(".he-lw", { ...props, }, [
        ...
        DetailsParameter({
          details,
          disabledReason: van.derive(() => !van.val(place) ? "Nejprve prosím zvolte odbìrové místo" : null),
      ...
      $("#product-detail-form").on("submit", function(e) {
        ...
        if(!van.val(place)) {
          showMessage("Vyberte prosím odbìrové místo.", "error");
          cancelEvent();
          return;
        }

        if(!van.val(details)) {
          showMessage("Vyplòte prosím osobní údaje vyšetøované osoby.", "error");
          cancelEvent();
          return;
        }
      });
    ...
    const LabItemDescription = ({ labItem, cartItem }) => {
      ...
      return h("div", [
        h(".he-lw-item__description-row", [
          h("strong", "Odbìrové místo: "),
          h("div", address),
        ]),
        h(".he-lw-item__description-row", [
          h("strong", "Vyšetøovaná osoba: "),
          h("div", `${name} ${surname}`),
          h("div", `${formatPhone(phone)}, ${email}`),
          h("div", `nar.:\u00a0${formatDob(dob)}, RÈ:\u00a0${personal_number}`),
    ...
    function transformRemark(remark, cartItems) {
      ...
      for(let { $tr, product, itemId, amount } of cartItems) {
        ...
        for(let [i, labItem] of labItems.entries()) {
          ...
          lines.push(`(${i + 1}.) Poèet vyšetøení: ${labItem.amount}`);
          lines.push(`> Odbìrové místo: ${address}`);
          lines.push(`> Vyšetøovaná osoba: `);
          lines.push(`> ${details.name} ${details.surname}`);
          lines.push(`> ${formatPhone(details.phone)}, ${details.email}`);
          lines.push(`> nar.:\u00a0${formatDob(details.dob)}, RÈ:\u00a0${details.personal_number}`);
  ...
  $(function() {
    ...
    $("input[type=search].form-control").attr("placeholder", "Co hledáte? Napø. hoøèík, lepší spánek...");

    $("<div>Potøebujete poradit? Zavolejte nám.</div>").prependTo(".header-top .project-phone");
    $("<a class='navigation-user --user' href='/login/'>Pøihlášení</a> <a class='navigation-user' href='/registrace/'>Registrace</a>").prependTo("body:not(.user-logged) #header .navigation-buttons");
    $("<a class='navigation-user --user' href='/login/'>Mùj úèet</a>").prependTo("body.user-logged #header .navigation-buttons");
    ...
    if($("body.id--15").length) {
      $(".order-summary-heading").text("Dìkujeme za objednávku");
      $("<p>Pokud jste objednali do 10:00, balíèek odešleme ještì ten samý den a dopravce doruèí následující den. Pokud byste mìli nìjaký problém nebo dotaz, volejte nám na <a href='tel:603 220 482'>603 220 482</a> nebo pište na <a href='mailto:info@herbsenergy.cz'>info@herbsenergy.cz</a></p>").insertAfter(".order-summary-heading");
    ...
    legacyLoadSnippets([
      ...
      $(".products-related-header").text("Další naše produkty s blahodárnými úèinky");
      ...
      if($ratingTab.length) {
        ...
        const $ratingTabTop = $("<div class='rating-tab__top'><div></div><span>Zákazníkù nás doporuèuje <a href='/recenze/'>Pøeèíst recenze</a></span></div>");
        if($(window).width() < 768) {
          // Mobile
          $ratingTab.prepend([
            $h(".ref__title.--title", "Naše hodnocení na Heurece"),
            $ratingTabTop,
            $h(".ref__title.--title", "Další hodnocení zákazníkù"),
          ]);
        } else {
          // Desktop
          $("<div class='ref__subtitle --subtitle'>Ovìøené recenze našich zákazníkù</div>").prependTo("#ratingTab");
      ...
      $("<a href='/klient/zapomenute-heslo/' class='forgot-pass'>Zapomnìli jste heslo?</a>").prependTo("#formLogin .password");
